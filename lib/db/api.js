...
dbapi.loadDevices = function(filter) {
  if (filter){
    var filterObject = {}
    var c = filter.split(':')
    var key = c[0]
    var value = c[1]
    //TODO: Better to be re-written with ES6 destructuring
    //      Wait for node/v8 to support this (node --v8-options | grep destructuring)
    ...
    return db.run(r.table('devices').filter(filterObject))
  } else {
