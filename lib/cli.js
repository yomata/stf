  ...
  .action(function(serials, options) {
    ...
    function run() {
      ...
      , procutil.fork(__filename, [
          ...
          , '--public-ip', options.publicIp
          , '--storage-url'
          , util.format('http://%s:%d/', localip, options.poorxyPort)
          , '--adb-host', options.adbHost
          , '--adb-port', options.adbPort
          , '--vnc-initial-size', options.vncInitialSize.join('x')
          ]
      ...
      , procutil.fork(__filename, [
          ...
          , '--app-url'
          , util.format('http://%s:%d/', localip, options.appPort)
          , '--auth-url'
          , util.format('http://%s:%d/', localip, options.authPort)
          , '--api-url'
          , util.format('http://%s:%d/', localip, options.apiPort)
          , '--websocket-url'
          , util.format('http://%s:%d/', localip, options.websocketPort)
          , '--storage-url'
          , util.format('http://%s:%d/', localip, options.storagePort)
          , '--storage-plugin-image-url'
          , util.format('http://%s:%d/', localip, options.storagePluginImagePort)
          , '--storage-plugin-apk-url'
          , util.format('http://%s:%d/', localip, options.storagePluginApkPort)
          ])
      ]
